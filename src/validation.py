...
def validate(configfile: Path):
    ...
    for d in config.data:
        ...
        if d.sheet_id and d.gid:
            asct_parsed = parse_asctb(load_asctb(gid=d.gid, sheet_id=d.sheet_id))
        if d.filename:
            asct_parsed = pd.read_csv(d.filename)

        report = run_validation(asct_parsed, config.relationships)

        report.to_csv(f"{data_dir}/{d.name}-{d.version}.tsv", sep="\t", index=False)


validate("validation-config.yml")
